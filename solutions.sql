...
SELECT books.title, editions.isbn, publsihers.name, stock.retail
FROM books
JOIN editions ON books.id = editions.book_id
JOIN stock ON editions.isbn = stock.isbn
...
FROM customers
JOIN shipment ON customers.id = shipments.customer_id
...
SELECT COUNT(editions.book_id), books.title, stock.stock
FROM editions
JOIN books ON editions.book_id = books.id
JOIN stock ON editions.isbn = stock.ISBN
